...
async def handle_contract(data: Dict[str, Any], filename: str):
  ...
  try:
    ...
    if not filename.endswith('.json'):
      raise ValueError(
        f"❌ Имя файла должно заканчиваться на .json, получено: {filename}")
    ...
    if not base_name.startswith("Договор_"):
      raise ValueError(
        f"❌ Имя файла должно начинаться с 'Договор_', получено: {base_name}")
    ...
    if not contract_template_path.exists():
      raise FileNotFoundError(
        f"❌ Шаблон договора не найден: {contract_template_path}")
    if not confirmation_template_path.exists():
      raise FileNotFoundError(
        f"❌ Шаблон подтверждения не найден: {confirmation_template_path}")
