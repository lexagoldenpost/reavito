...
class TelegramSender:
    ...
    async def send_message_async(
        ...
        async with self.client:
            return await self._send_message_async(channel_name, message, media_files)

    async def get_channel_id(self, channel_username: str):
        """Получить ID канала по username"""
        async with self.client:
            try:
                entity = await self.client.get_entity(channel_username)
                print(f"ID канала {channel_username}: {entity.id}")
                return entity.id
            except Exception as e:
    ...
    def send_message(
        ...
        return self.client.loop.run_until_complete(
            self.send_message_async(channel_name, message, media_files)
        )
...
if __name__ == "__main__":
    ...
    channel_id = sender.client.loop.run_until_complete(
        sender.get_channel_id("@bookind_data")
    )
    print(f"ИД канала {channel_id}")

    # Пример 1: Только текст
    print("Отправка текстового сообщения:")
    result = sender.send_message(
        channel_name="-1002679682284",   #@bookind_data
