...
class TelegramClientManager:
    ...
    def _get_unique_session_name(self) -> str:
        """
        Генерирует уникальное имя сессии для текущего процесса.
        Использует имя процесса и PID для уникальности.
        """
        base_name = f"{self.api_id}_{Config.TELEGRAM_SESSION_NAME}"

        # Получаем имя процесса
        process_name = multiprocessing.current_process().name

        # Заменяем недопустимые символы для имени файла
        process_name = process_name.replace(' ', '_').replace('-', '_')

        # Если процесс не имеет имени (например, главный), используем 'main'
        if not process_name or process_name == 'MainProcess':
            process_name = 'main'

        return f"{base_name}_{process_name}"
    ...
    def _initialize(self):
        ...
        # Определяем путь к файлу сессии с уникальным именем для процесса
        session_filename = self._get_unique_session_name()
        ...
        logger.info(f"📁 Используется сессия: {self.session_file_path.name}")
    ...
    async def ensure_connection_lazy(self) -> bool:
        """
        Ленивая инициализация - подключается только если нужно,
        но не выводит ошибки если сессии нет.
        """
        try:
            if self._connection_open and await self.client.is_user_authorized():
                return True
            ...
            if self.session_file_path.exists():
                ...
                if await self.client.is_user_authorized():
                    logger.info(f"✅ Используем существующую сессию: {self.session_file_path.name}")
                    return True

            # Нет сессии - пытаемся создать новую
            logger.info("🔑 Нет активной сессии, запускаем авторизацию...")
            return await self.ensure_connection()

        except Exception as e:
            logger.warning(f"⚠️ Ленивая инициализация не удалась: {e}")
            return False
